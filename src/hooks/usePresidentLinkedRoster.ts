...
type RemotePayload = {
  ...
  playerTopStats: PresidentPlayerTopStat[];
...
export function usePresidentLinkedRoster() {
  ...
  const selfSnapshot = useMemo(
    () =>
      buildWorkspaceSnapshotV1({
        ...
        leagueSetup: app.leagueSetup,
        pastClubResults: app.pastClubResults,
        ...
        try {
          ...
          if (res.ok && data.ok === true) {
            setRemote({
              ...
              playerTopStats: (data.playerTopStats as PresidentPlayerTopStat[]) ?? [],
  ...
  const merged = useMemo(() => {
    if (!user?.id || !isClubPresident(user)) {
      return {
        ...
        playerTopStats: [] as PresidentPlayerTopStat[],
    ...
    if (remote && (remote.coaches.length > 0 || remote.players.length > 0)) {
      ...
      return {
        ...
        playerTopStats: remote.playerTopStats,
    ...
    const playerTopStats = mapWorkspaceToPresidentPlayerTopStats(user.id, user.email ?? "", selfSnapshot);
    const hasSelf =
  ...
  return {
    ...
    playerTopStats: merged.playerTopStats,
