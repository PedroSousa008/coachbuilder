# Training Exercise Images (PDF only)

These images are used only in generated print/PDF documents from Training Plans.
They are not rendered in the normal website UI.

Expected file names:

- `warm-up.png`
- `passing-activation.png`
- `dual-passing.png`
- `rondo-pressao-condicionada.png`
- `jogo-4x4-transicao-imediata.png`
- `pressao-alta-coordenada-5v5-1.png`
- `finalizacao-velocidade-cruzamento.png`
- `back-four-shifting.png`
- `compact-defending-transition.png`
- `possessao-8v8-2-neutros.png`
- `offensive-between-lines.png`
- `between-the-lines.png`
- `9v9-2-game.png`
- `double-finishing-drill.png`
- `finishing-transition.png`
- `cross-and-strike.png`
- `4-finishing-drills.png`
- `rondo-9v3.png`
- `rondo-5v3.png`
- `breakout-rondo.png`
- `build-up-into-counter-attack.png`
- `fitness-rondo-finishing.png`
- `rondo-to-counter.png`
- `goal-kick-1.png`
- `goal-kick-2.png`
- `midfielder-run-behind-defense.png`
- `full-back-overlap-winger.png`
- `full-back-overlap-striker.png`
- `3v2-fast-break.png`
- `3v2-finishing-drill.png`
- `3v3-5-teams.png`
- `fixed-position-rondo.png`
- `aquecimento-com-movimentação.png`
- `2+1v1-transition.png`
- `short-corner.png`
- `short-corner-newcastle.png`
- `pressing-exercise.png`
- `defensive-recovery.png`
